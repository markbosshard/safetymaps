...
Basemap tiles and search geocoding use **MapTiler** — account **mark.bosshard4@gmail.com** (manage,
rotate, or set usage limits on the key there). Put your key in a gitignored `maptiler.key` file at the
repo root (or set `MAPTILER_KEY` in the env); `npm run build` injects it into `index.html`.
